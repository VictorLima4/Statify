...
export default function TopTracks() {
  const [timeRange, setTimeRange] = useState<GetTopTracksTimeRange>("long_term");
      ...
      {isLoading ? (
        ...
        <div className="space-y-1">
          {(tracks as any[])?.map((track: any, index: number) => (
            <Link key={`${track.id}-${index}`} href={`/track/${track.id}`}>
              <div className="flex items-center gap-4 p-3 rounded-lg hover:bg-secondary/30 transition-colors group cursor-pointer">
                ...
                <div className="hidden md:block w-1/4 min-w-0 px-4">
                  <p className="text-sm text-muted-foreground truncate">{track.album?.name ?? ""}</p>
                </div>

                {track.streamCount != null ? (
                  <div className="text-right min-w-[5rem]">
                    <p className="text-sm font-semibold text-primary">{track.streamCount.toLocaleString()}</p>
                    <p className="text-xs text-muted-foreground">plays</p>
                  </div>
                ) : (
                  <div className="text-sm text-muted-foreground font-mono w-12 text-right">
